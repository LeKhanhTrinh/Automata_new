...
public class ArrowTool extends Tool {
	...
	public void mouseDragged(MouseEvent event) {
		...
		if (lastClickedState != null) {
			...
			if (f != t) {
				...
				// angle-Math.PI*.166);
				Transition[] trans = getAutomaton()
						.getTransitionsFromStateToState(f, t);
				for (int n = 0; n < trans.length; n++) {
					CurvedArrow arrow = (CurvedArrow) getView().getDrawer().transitionToArrowMap
							.get(trans[n]);
			...
			if (nowY < initialPointClick.y)
				topY = nowY;
			bounds = new Rectangle(leftX, topY, Math.abs(nowX
					- initialPointClick.x),
		...
		if (selectedTransition != null) { // simply set ...but we need to get
			...
			CurvedArrow ca = (CurvedArrow) getView().getDrawer().transitionToArrowMap
			...
			if (transitionInFlux
					|| Math.sqrt((control.getX() - myClickP.x)
							* (control.getX() - myClickP.x)
	...
	protected class StateMenu extends JPopupMenu implements ActionListener {
		public StateMenu() {
			...
			setName = new JMenuItem("Change Label");
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (item == makeFinal) {
				...
				while (((TuringMachine) parent.getAutomaton()).getParent() != null) {
					parent = ((TuringMachine) parent.getAutomaton())
							.getParent();
				}
				EditBlockPane editor = new EditBlockPane(
						((TMState) state).getInnerTM());

				EnvironmentFrame rootFrame = parent.getAutomaton()
						.getEnvironmentFrame();
				...
				TMState buffer = ((TuringMachine) getAutomaton())
						.createTMState(new Point(state.getPoint().x + 4, state
								.getPoint().y));
				buffer.setInnerTM((TuringMachine) ((TMState) state)
						.getInnerTM().clone());
				...
				Object newString = JOptionPane.showInputDialog(null,
						"Replace With");
				if (newString == null)
					return;
				...
				replaceCharactersInBlock((TMState) state, toReplace,
						replaceWith);
			}
		...
		private void replaceCharactersInBlock(TMState start, String toReplace,
				String replaceWith) { // this shall be a recursive method,
										// replacing the inside and then the out

			TuringMachine tm = start.getInnerTM();

			for (int i = 0; i < tm.getStates().length; i++)
				replaceCharactersInBlock((TMState) tm.getStates()[i],
			...
			for (int i = 0; i < trans.length; ++i) {
				...
				for (int k = 0; k < tmTrans.tapes(); k++) {
					...
					tmTrans.setWrite(k,
							write.replaceAll(toReplace, replaceWith));
				}
		...
		private JMenuItem changeLabel, deleteLabel, deleteAllLabels, editBlock,
				copyBlock, replaceSymbol, setName;
	}
