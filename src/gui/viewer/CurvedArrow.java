...
public class CurvedArrow {
	...
	public void draw(Graphics2D g) {
		...
		State s = myTransition.getToState();
		int dx = (int) g.getFontMetrics().getStringBounds(s.getName(), g)
				.getWidth() >> 1;
		int dy = ((int) g.getFontMetrics().getAscent()) >> 1;
		drawArrow(g, end, control, dx); // Draws the arrow head.
		drawText(g);
	}

	public void drawControlPoint(Graphics2D g) { // adjust later to center of
													// circle = focus point
		g.drawOval((int) curve.getCtrlX() - 5, (int) curve.getCtrlY() - 5, 10,
				10);
	}
	...
	public void drawText(Graphics2D g) {
		...
		for (int i = 0; i < label.length(); i += CHARS_PER_STEP) {
			String sublabel = label.substring(i,
					Math.min(i + CHARS_PER_STEP, label.length()));
	 ...
	 */
	private void drawArrow(Graphics g, Point head, Point away, int dx) {
		int endX, endY;
		double angle = Math.atan2((double) (away.x - head.x),
				(double) (away.y - head.y));
		int _dx = (int) (Math.sin(angle) * dx);
		int _dy = (int) (Math.cos(angle) * dx - 1/2);
		angle += ARROW_ANGLE;
		endX = ((int) (Math.sin(angle) * ARROW_LENGTH)) + head.x + _dx;
		endY = ((int) (Math.cos(angle) * ARROW_LENGTH)) + head.y + _dy;
		
		g.drawLine(head.x + _dx, head.y + _dy, endX, endY);
		angle -= 2 * ARROW_ANGLE;
		endX = ((int) (Math.sin(angle) * ARROW_LENGTH)) + head.x + _dx;
		endY = ((int) (Math.cos(angle) * ARROW_LENGTH)) + head.y + _dy;
		g.drawLine(head.x + _dx, head.y + _dy, endX, endY);
