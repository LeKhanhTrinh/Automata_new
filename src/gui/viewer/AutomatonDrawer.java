...
public class AutomatonDrawer {
	...
	public Rectangle getBounds() {
		...
		return cachedBounds = curTransform.createTransformedShape(rect)
				.getBounds();
	}
			...
			for (int j = i + 1; j < states.length; j++) {
				...
				for (int n = 0; n < itoj.length; n++) {
					...
					CurvedArrow arrow = n == 0 ? new CurvedArrow(fromI, fromJ,
							curvy, itoj[n]) : new InvisibleCurvedArrow(fromI,
							fromJ, curvy, itoj[n]);
				...
				for (int n = 0; n < jtoi.length; n++) {
					...
					CurvedArrow arrow = n == 0 ? new CurvedArrow(fromJ, fromI,
							curvy, jtoi[n]) : new InvisibleCurvedArrow(fromJ,
							fromI, curvy, jtoi[n]);
			...
			for (int n = 0; n < trans.length; n++) {
				if (selfTransitionMap.containsKey(trans[n])) {
					...
					Point storedto = pointOnState(states[i],
							(selfTransitionMap.get(trans[n]) - Math.PI * .166));
					CurvedArrow arrow = n == 0 ? new CurvedArrow(storedfrom,
							storedto, -2.0f, trans[n])
							: new InvisibleCurvedArrow(storedfrom, storedto,
									-2.0f - n, trans[n]);
					...
					CurvedArrow arrow = n == 0 ? new CurvedArrow(from, to,
							-2.0f, trans[n]) : new InvisibleCurvedArrow(from,
							to, -2.0f - n, trans[n]);
	...
	public State stateAtPoint(Point point) {
		...
		for (int i = states.length - 1; i >= 0; i--) {
			int dx = ((int) g.getFontMetrics()
					.getStringBounds(states[i].getName(), g).getWidth()) >> 1;
			if (point.distance(states[i].getPoint()) <= StateDrawer.STATE_RADIUS + dx) {
